...
__global__ void squareSort(T* data, int N) {
  ...
  for(int sec = 0; sec < (N/gridDim.x); sec += M*(blockDim.x/W)) {
    for(int i=0; i<ELTS; i++) {
      regs[i] = data[blockOffset + sec + (i*blockDim.x) + threadIdx.x];
    }
...
// transpose in shared memory then write to global
    ...
    for(int i=(blockDim.x/W)-1; i>warpId; i--) // Wait for other warps to catch back up
      __syncthreads();
  }
}
...
/* Main basecase sorting Kernel */
template<typename T, fptr_t f>
__device__ void squareSortDevice(T* data, int N) {
  T regs[ELTS];
